...
import { CreateUserBodyType, GetUsersQueryType, UpdateUserBodyType } from 'src/modules/user/user.schema'
...
} from 'src/common/helpers/prisma-error'
...
export class UserService {
  constructor(
    ...
    private commonRolesRepository: CommonRolesRepository,
  ) {}
  ...
  private async verifyRole({ roleNameAgent, roleIdTarget }) {
    ...
    if (roleNameAgent === RoleName.Admin) {
      ...
      const adminRoleId = await this.commonRolesRepository.getAdminRoleId()
  ...
  async update({
    ...
    try {
      ...
      const updatedUser = await this.commonUserRepository.update(
        { id },
        {
          ...data,
          updatedById,
  ...
  private async getRoleIdByUserId(userId: number) {
    const currentUser = await this.commonUserRepository.findUnique({
      id: userId,
    })
    if (!currentUser) {
      throw NotFoundRecordException
